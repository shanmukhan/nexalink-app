...
## MVP
- Backend (`nexalink-api`, sibling repo): auth/OTP, customer/address, product (read-only), cart + checkout + order lifecycle, wallet, referral — **implemented and manually verified end-to-end** (M0–M3). `pack`/`ship`/`deliver` and wallet withdrawal approve/reject/pay have no HTTP route yet — deferred to M4 RBAC. Coupon module (Phase 6, beyond the original M0-M5 milestone plan) added net-new: admin-managed codes, cart-preview validation, and enforcement at checkout.
- Customer App
